...
func TestTokenFilterConfig(t *testing.T) {
	testCases := []struct {
		name       string
		setup      func()
		assertions func(webhooks.TokenFilterConfig, error)
	}{
		{
			name: "TOKENS_PATH not set",
			assertions: func(_ webhooks.TokenFilterConfig, err error) {
				require.Error(t, err)
				require.Contains(t, err.Error(), "value not found for")
		...
		},
		{
			...
			setup: func() {
				os.Setenv("TOKENS_PATH", "/completely/bogus/path")
			},
			assertions: func(_ webhooks.TokenFilterConfig, err error) {
				require.Error(t, err)
				require.Contains(
		...
		},
		{
			...
			setup: func() {
				...
				require.NoError(t, err)
				os.Setenv("TOKENS_PATH", tokensFile.Name())
			},
			assertions: func(_ webhooks.TokenFilterConfig, err error) {
				require.Error(t, err)
				require.Contains(
		...
		},
		{
			...
			assertions: func(config webhooks.TokenFilterConfig, err error) {
				require.NoError(t, err)
			},
		},
	}
