...
build:
	$(KANIKO_DOCKER_CMD) kaniko \
		...
		--dockerfile /workspaces/brigade-dockerhub-gateway/Dockerfile \
		--context dir:///workspaces/brigade-dockerhub-gateway/ \
		--no-push
...
################################################################################
# Targets to facilitate hacking on Brigade Docker Hub Gateway.                 #
################################################################################
...
.PHONY: hack-deploy
hack-deploy:
	helm dep up charts/brigade-dockerhub-gateway && \
	helm upgrade brigade-dockerhub-gateway charts/brigade-dockerhub-gateway \
		--install \
		--create-namespace \
